function [S,varargout] = ...
    getNDexpansion(R,ND,order_fit,r_n,InxCap_fit,varargin)

%% Input handling
InxBottom_fit = 2;
if length(varargin) >= 1
    InxBottom_fit = varargin{1};
end
c = NaN;
a = NaN;
...
%     icap = 155;
    icap = InxCap_fit(fi);
    if icap - InxBottom_fit + 1 <= order_fit
        error('Not enough data point for fit.')
    end
...
xline(fx_plot(Rcap_best),'r-');
h_fit = plot(fx_plot(R.'),fitfun(fi_best,R.'.^2),'r-');

% residuals
subplot(order_fit+3+1,1,3); hold on;
% h(end+1) = plot(x_fit,y_fit,'k.-'); ax(1) = h(1).Parent;
residual = ND./R.^2 - fitfun(fi_best,R.'.^2).';
...
%% output
fname = 'temp_getNDexpansion.mat';
% save(fname);
% save(fname,'-regexp','^(?!(variableToExclude1|variableToExclude2)$). ')
save(fname,'-regexp','^(?!(polyexp_fig|ax|h)$).')
S = load(fname);
delete(fname)

varargout{1} = polyexp_fig;